"""
工具定义模块 - 为Docker智能体定义可用的函数工具

本模块定义了智能体可以调用的三个核心工具：
1. str_replace_editor: 文件编辑工具（查看、创建、编辑文件）
2. execute_bash: Bash命令执行工具
3. submit: 任务提交工具
"""

# str_replace_editor支持的命令列表
ALLOWED_STR_REPLACE_EDITOR_COMMANDS = ["view", "create", "str_replace", "insert"]

# str_replace_editor工具的描述文档
...
# str_replace_editor工具定义 - 用于查看、创建和编辑文件
...
# execute_bash工具定义 - 用于在终端执行Bash命令
execute_bash_tool = {
    "type": "function",
    "function": {
        "name": "execute_bash",
