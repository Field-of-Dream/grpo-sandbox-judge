...
from grpo_in_sandbox.benchmark.biomed.reward import compute_score as compute_biomed_score
from grpo_in_sandbox.benchmark.chem.reward import compute_score as compute_chem_score
from grpo_in_sandbox.benchmark.instruct_follow.reward import compute_score as compute_if_score
from grpo_in_sandbox.benchmark.math.reward import compute_score as compute_math_score

# ROUGE for open-ended questions
try:
    from rouge_score import rouge_scorer
except ImportError as err:
    raise ImportError("rouge-score is required. Install with: pip install rouge-score") from err
...
def compute_score(agent_answer: str, ground_truth: str, **kwargs) -> float:
    ...
    if standardized_type == "multiple_choice_single_answer":
        ...
        model_choices = {c.strip().upper() for c in model_output.split(',') if c.strip()}
        gt_choices = {c.strip().upper() for c in ground_truth.split(',') if c.strip()}
